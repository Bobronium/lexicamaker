...
class IOBridge:
    ...
    def open_files(self):
        ...
        if self.annotationFile :
            ...
            try:
                self.annotationFile = open(os.path.join(dictionaryPath, dictionaryName) + '.ann', 'r', encoding=self.encoding)
        ...
        if self.abbreviationsFile :
            ...
            try:
                self.abbreviationsFile = open(os.path.join(dictionaryPath, dictionaryName) + '_abrv.dsl', 'r', encoding=self.encoding)
            except FileNotFoundError:
                if self.verbose >= 2:
